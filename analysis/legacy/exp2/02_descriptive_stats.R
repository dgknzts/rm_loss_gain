# ==============================================================================
# REDUNDANCY MASKING STUDY - EXP2 DESCRIPTIVE STATISTICS
# ==============================================================================
# This script generates comprehensive descriptive statistics for exp2
# Input: Processed dataset from data/exp2/processed.csv
# Output: Descriptive tables saved to outputs/exp2/tables/
# NOTE: Exp2 has only width adjustments (no spacing)
# ==============================================================================

library(tidyverse)
library(knitr)

cat("Generating descriptive statistics for Exp2...\n\n")
...
# Key outcome variables for analysis (width-only for exp2)
outcome_vars <- c(
  "width_deviation", "pooled_width_deviation",
  "width_deviation_relative", "absolute_width_deviation"
)
...
design_balance <- df_rm %>%
  ...
  count(RM_condition, correct_num, correct_width) %>%
  arrange(RM_condition, correct_num, correct_width)
...
write.csv(descriptives, "outputs/exp2/tables/descriptive_statistics_by_condition.csv", row.names = FALSE)

# Save design balance table
write.csv(design_balance, "outputs/exp2/tables/experimental_design_balance.csv", row.names = FALSE)
...
write.csv(sample_summary, "outputs/exp2/tables/sample_characteristics.csv", row.names = FALSE)

# Save RM condition distribution
write.csv(rm_distribution, "outputs/exp2/tables/rm_condition_distribution.csv", row.names = FALSE)

cat("\nDescriptive analysis complete!\n")
cat("Tables saved to outputs/exp2/tables/:\n")
