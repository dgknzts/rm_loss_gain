...
spacing_emm_summary <- summary(emm_spacing_setsize)
spacing_contrast_summary <- summary(spacing_dev_contrasts, infer = TRUE)
spacing_contrast_summary <- add_ci_columns(spacing_contrast_summary)

write.csv(as.data.frame(spacing_emm_summary),
          'outputs/tables/spacing_deviation_emmeans_by_spacing_setsize.csv', row.names = FALSE)
write.csv(as.data.frame(spacing_contrast_summary),
          'outputs/tables/spacing_deviation_rm_contrasts.csv', row.names = FALSE)
...
means_wide <- as_tibble(spacing_emm_summary) %>%
  select(spacing_category, correct_num, rm_type, emmean, SE) %>%
  pivot_wider(names_from = rm_type, values_from = c(emmean, SE), names_sep = '_')

contrast_table <- as_tibble(spacing_contrast_summary) %>%
  mutate(
  ...
  left_join(means_wide, by = c('spacing_category', 'correct_num'))
...
cat(sprintf('RM vs NoRM effects were evaluated across %d spacing-category x set-size cells. Significant FDR-adjusted effects appeared in %d of %d cells (%s%%).\n',
            n_conditions, significant_conditions, n_conditions, format_num(percent_significant, digits = 1)))

if (n_conditions > 0) {
  cat(sprintf('Strongest effect: spacing %s, set size %s (RM - NoRM = %s, z = %s, %s; FDR %s, d = %s, 95%% CI [%s, %s]).\n\n',
              strongest_effect$spacing_category, strongest_effect$correct_num,
  ...
  apply(contrast_table, 1, function(row) {
    cat(sprintf('Spacing %s, set size %s: RM (M = %s, SE = %s) vs NoRM (M = %s, SE = %s), z = %s, %s (FDR %s), d = %s, 95%% CI [%s, %s].\n',
...
cat('\nSpacing deviation analysis complete. Copy the sentences above directly into the manuscript.\n')
