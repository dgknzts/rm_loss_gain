...
analysis_data <- df %>%
  ...
  filter(is.finite(spacing_deviation_relative))

# --- Bayesian model ---
set.seed(1234)
model <- stan_lmer(
  spacing_deviation_relative ~ rm_type * spacing_category * correct_num * correct_width + (1 | subID),
  data = analysis_data,
...
write.csv(posterior_means, 'outputs/tables/relative_spacing_deviation_posterior_means.csv', row.names = FALSE)
write.csv(posterior_difference, 'outputs/tables/relative_spacing_deviation_posterior_difference.csv', row.names = FALSE)
write.csv(bf_means_tbl, 'outputs/tables/relative_spacing_deviation_bayes_vs_zero.csv', row.names = FALSE)
write.csv(bf_difference_tbl, 'outputs/tables/relative_spacing_deviation_bayes_difference.csv', row.names = FALSE)
...
bayes_density_plot <- ggplot(emm_draws, aes(x = estimate, fill = rm_type, color = rm_type)) +
  ...
  labs(
    title = 'Posterior means: relative spacing deviation',
    x = 'Relative spacing deviation',
    y = 'Density',
    fill = 'Condition',
    color = 'Condition'
  ) +
...
ggsave(
  filename = 'outputs/figures/relative_spacing_deviation_posterior_density.png',
  plot = bayes_density_plot,
  width = 6,
  height = 4,
  dpi = 300
)
