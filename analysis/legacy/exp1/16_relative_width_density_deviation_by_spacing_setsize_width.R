# Relative Width Density Deviation by Spacing × Set Size × Width (All 3 Factors)
...
write.csv(as.data.frame(summary(emm_all)),
          'outputs/exp1/tables/relative_width_density_deviation_emmeans_by_spacing_setsize_width.csv',
          row.names = FALSE)

contrast_summary <- summary(contrasts, infer = TRUE)
write.csv(as.data.frame(contrast_summary),
          'outputs/exp1/tables/relative_width_density_deviation_rm_contrasts_by_spacing_setsize_width.csv',
          row.names = FALSE)
...
density_plot <- plot_data %>%
  ...
  labs(title = "Relative Width Density Deviation by Spacing × Set Size × Width",
       subtitle = "All three experimental factors combined, normalized by actual density") +
...
ggsave("outputs/exp1/figures/relative_width_density_deviation_by_spacing_setsize_width.png",
       density_plot, width = 11, height = 9, dpi = 300, bg = "white")

cat("\nSaved: outputs/exp1/figures/relative_width_density_deviation_by_spacing_setsize_width.png\n")
