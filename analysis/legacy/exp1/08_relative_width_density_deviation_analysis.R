# Relative Width Density Deviation Statistical Analysis and Visualization
# Mixed-effects model with emmeans contrasts + publication-ready figure
# Relative measure: width_density_deviation / actual_width_density
...
analysis_data <- df %>%
  ...
  mutate(
    ...
    # Calculate relative width density deviation
...
write.csv(as.data.frame(summary(emm_density_setsize)),
          'outputs/exp1/tables/relative_width_density_deviation_emmeans_by_width_setsize.csv',
          row.names = FALSE)

density_contrast_summary <- summary(density_dev_contrasts, infer = TRUE)
write.csv(as.data.frame(density_contrast_summary),
          'outputs/exp1/tables/relative_width_density_deviation_rm_contrasts.csv',
          row.names = FALSE)
...
# Calculate participant means for relative width density deviation
...
density_plot <- plot_data %>%
  ...
  labs(title = "Relative Width Density Deviation by True Width and Set Size",
       subtitle = "Normalized by actual width density") +
...
ggsave(
  filename = "outputs/exp1/figures/relative_width_density_deviation_analysis.png",
  plot = density_plot,
  width = 9,
  height = 5,
  dpi = 300,
...
ggsave(
  filename = "outputs/exp1/figures/relative_width_density_deviation_analysis.pdf",
  plot = density_plot,
  width = 9,
  height = 5,
  units = "in",
...
cat("- relative_width_density_deviation_emmeans_by_width_setsize.csv\n")
cat("- relative_width_density_deviation_rm_contrasts.csv\n")
cat("\nFigures saved to outputs/exp1/figures/:\n")
cat("- relative_width_density_deviation_analysis.png\n")
cat("- relative_width_density_deviation_analysis.pdf\n")
