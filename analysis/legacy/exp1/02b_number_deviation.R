# Participant-specific number deviation analysis - Exp1
...
cat(sprintf("Exp1 data: %d participants, %d trials\n",
...
plot_deviation <- ggplot(deviation_data,
  ...
  geom_point(position = position_jitter(width = 0.15, seed = 123),
             shape = 21, fill = "white", stroke = 0.8, size = 2.5, alpha = 0.7) +
  geom_errorbar(data = grand_avg,
                aes(x = factor(correct_num), y = grand_mean,
                    ymin = ci_lower, ymax = ci_upper),
                width = 0.01, linewidth = 1.2, color = "black", inherit.aes = FALSE) +
...
ggsave("outputs/exp1/figures/participant_number_deviation.png",
       plot_deviation, width = 8, height = 6, dpi = 300, bg = "white")
# ggsave("outputs/exp1/figures/participant_number_deviation.pdf",
#        plot_deviation, width = 8, height = 6, bg = "white")
cat("Saved: outputs/exp1/figures/participant_number_deviation.png\n")
cat("Saved: outputs/exp1/figures/participant_number_deviation.pdf\n")