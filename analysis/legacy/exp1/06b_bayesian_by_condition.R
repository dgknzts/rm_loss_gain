# Bayesian One-Sample T-Tests by Width x Set Size x RM Type - Exp1
...
# Load data
df <- read_csv("data/exp1/processed.csv", show_col_types = FALSE)
...
heatmap_plot <- ggplot(results, aes(x = factor(width), y = factor(setsize), fill = bf_01)) +
  ...
  labs(
    title = "Bayesian Evidence for Null (No Width Bias)",
...
forest_plot <- ggplot(results_forest, aes(x = median_delta, y = condition, color = rm_type)) +
  ...
  labs(
    title = "Effect Sizes (Cohen's δ) by Condition",
...
if (!dir.exists("outputs/exp1/figures")) dir.create("outputs/exp1/figures", recursive = TRUE)
if (!dir.exists("outputs/exp1/tables")) dir.create("outputs/exp1/tables", recursive = TRUE)

ggsave("outputs/exp1/figures/bayesian_heatmap_by_condition.png",
       heatmap_plot, width = 9, height = 5, dpi = 300, bg = "white")

ggsave("outputs/exp1/figures/bayesian_forest_by_condition.png",
       forest_plot, width = 10, height = 6, dpi = 300, bg = "white")

ggsave("outputs/exp1/figures/bayesian_by_condition_combined.png",
...
write_csv(results_export, "outputs/exp1/tables/bayesian_by_condition_results.csv")

cat("Exp1 Bayesian analysis by condition complete.\n")
cat("Saved: outputs/exp1/figures/bayesian_heatmap_by_condition.png\n")
cat("Saved: outputs/exp1/figures/bayesian_forest_by_condition.png\n")
cat("Saved: outputs/exp1/figures/bayesian_by_condition_combined.png\n")
cat("Saved: outputs/exp1/tables/bayesian_by_condition_results.csv\n")
